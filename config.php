...
$pathA = explode("/", ($_SERVER['SCRIPT_NAME']));
// Remove index.php
array_pop($pathA);	//remove comment before committing
// Application path
$path = join("/", $pathA);

// Full uri with server
$base_url                       = $protocol . $_SERVER['HTTP_HOST'] . $path . "/";
$base_path                      = $_SERVER['DOCUMENT_ROOT'] . (!empty($path) ? $path . '/': '/');
$system_folder                  = "admin.php";
...
$multimedia_path		= $base_path . $multimedia_folder;
$multimedia_url			= $base_url . $multimedia_folder;
...
$config['tmpl_file_basepath']   = $base_path . "themes/";
